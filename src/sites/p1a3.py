...
POST_QUESTION_URL = "https://api.1point3acres.com/api/daily_questions"
...
def daily_checkin(
    ...
    result = solver.turnstile(
        sitekey=CF_CAPCHA_SITE_KEY, url=CHECKIN_URL, useragent=USER_AGENT
    )
    if result is None:
        raise ValueError("Failed to get capcha")
    ...
    response = s.post(POST_CHECKIN_URL, headers=headers, data=json.dumps(body))
...
def answer_daily_question(
    ...
    result = solver.turnstile(
        sitekey=CF_CAPCHA_SITE_KEY, url=QUESTION_URL, useragent=USER_AGENT
    )
    if result is None:
        raise ValueError("Failed to get capcha")
    ...
    headers = common_header()
    response = s.post(POST_QUESTION_URL, headers=headers, data=json.dumps(body))
    if "人机验证出错，请重试" in response.text:
        solver.report(captcha_id, False)
        raise ValueError("The captcha is wrong")
    ...
    if result["errno"] == 0:
        ...
        logger.warning(f"Unknown error: {result['errno']}: {response.text}")
...
def get_daily_task_answer(
    ...
    headers = common_header()
    response = s.get(POST_QUESTION_URL, headers=headers)
    ...
    logger.info(f"The options are: {answers}")
    ...
    if question in questions.keys():
        ...
        for k in answers:
            if answers[k] in answer:
                ...
                answer_id = k
        if answer_id == "":
            raise ValueError(f"The question: {question} and answer not found: {answer}")
    else:
